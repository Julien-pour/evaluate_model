...
parser.add_argument("-d", "--arg_selction_dataset", type=str, help=" train on first example generated by the archive or last",default="first")
...
selection_dataset = args.arg_selction_dataset
...
if seed[0]=="1" or seed[0]=="2" or seed[0]=="3":
    path_rd_gen= "run_saved/"+option+"/maps_"+seed[0]+"_rd_gen.json"
    path_elm="run_saved/"+option+"/maps_"+seed[0]+"_elm.json"
    path_elm_nlp="run_saved"+option+"/maps_"+seed[0]+"_elm_NLP.json"
    path_img_rd="run_saved/"+option+"/maps_"+seed[0]+"_imgep_random.json"
    path_imgp_smart="run_saved/"+option+"/maps_"+seed[0]+"_imgep_smart.json"
...
path_save=args.base_path+"save_results/"+option+"/"+name+".json"
print("\n=============\n ")
...
len_P3_train=len(dataset_r)


dataset = load_dataset("json", data_files=path_train, split="train")
sorted_dataset = dataset.sort("idx_generation")
if selection_dataset=="first":
    n_puzzle2select = 2500 - len_P3_train
    list_idx = [i for i in range(n_puzzle2select)]
    sorted_dataset=sorted_dataset.select(list_idx)
if selection_dataset=="last":
    n_puzzle2select = 2500 - len_P3_train
    list_idx = [i for i in range(len(sorted_dataset)-n_puzzle2select,len(sorted_dataset))]
    sorted_dataset=sorted_dataset.select(list_idx)
cat_datasets=concatenate_datasets([sorted_dataset,dataset_r])
...
name_json=args.base_path+"save_results/"+selection_dataset+"_"+name+model_id#.split("/")[1]
name_json_sol=args.base_path+"save_sol/"+selection_dataset+"_"+name+model_id#.split("/")[1]
...
if train_model:
    ...
    output_dir = selection_dataset+"_"+model_id+name+"e_"+str(num_train_epochs)+"_"+str(seed) #args.base_path+"hf/datasets"+name # where to save model
    trainer.save_model(output_dir)

if eval_model:  # OOD
    output_dir = selection_dataset+"_"+model_id+name+"e_"+str(num_train_epochs)+"_"+str(seed)    # del model
    # del tokenizer
    # gc.collect()
