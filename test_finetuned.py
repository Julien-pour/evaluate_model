...
parser.add_argument("-p", "--arg_path_idx", type=int, help="path baseline idx  (data to use as trainset)")
...
parser.add_argument("-f", "--arg_limited_trainset", type=str, help=" model",default=True)
...
cat_datasets = dataset.shuffle(seed=42) #cat_datasets.shuffle(seed=42)
...
hf_dir=args.base_path+"hf/models/"

path_load_model=hf_dir+model_id
print("path_load_model",path_load_model)
name_json=args.base_path+"save_results/"+option+"/"+name+model_id#.split("/")[1]
name_json_sol=args.base_path+"save_sol/"+option+"/"+name+model_id#.split("/")[1]

if train_model:
    tokenizer = LlamaTokenizer.from_pretrained(path_load_model,local_files_only=True)
    ...
    output_dir = hf_dir+option+model_id+name+"e_"+str(num_train_epochs)+"_"+str(seed) #args.base_path+"hf/datasets"+name # where to save model
    trainer.save_model(output_dir)

if eval_model:  # OOD
    output_dir = hf_dir+option+model_id+name+"e_"+str(num_train_epochs)+"_"+str(seed)
    # del model
    # del tokenizer
    # gc.collect()
